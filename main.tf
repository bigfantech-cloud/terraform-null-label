locals {
  ...
  defaults = {
    label_order         = ["project_name", "aws_estate", "environment", "stage", "attributes"]
  ...
  input = {
    ...
    aws_estate   = var.aws_estate == null ? lookup(var.context, "aws_estate", null) : var.aws_estate
  ...
  string_label_names = ["project_name", "aws_estate", "environment", "stage", "name"]
  ...
  project_name = local.formatted_labels["project_name"]
  aws_estate   = local.formatted_labels["aws_estate"]
  environment  = local.formatted_labels["environment"]
  stage        = local.formatted_labels["stage"]
  namespace    = local.formatted_labels["project_name"]
